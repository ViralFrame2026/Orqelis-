...
import { OrqelisAiDemo } from "@/components/demo/orqelis-ai-demo";
...
export default async function DemoPage() {
  ...
  const aiDemoProducts = products.slice(0, 12).map((product) => ({ name: product.name, price: product.price, stock: product.stock, status: product.status }));
  return <>
    <div className="bg-clay px-4 py-2 text-center text-xs font-extrabold text-white">DEMO ORQELIS · Esta tienda es una demostración del sistema <Link href="/" className="ml-2 underline">Conocer ORQELIS</Link> · <Link href="#ia-demo" className="ml-2 underline">Probar IA</Link></div>
    ...
    <OrqelisAiDemo products={aiDemoProducts} />
    <section className="container-store py-16 sm:py-24"><SectionHeading eyebrow="Experiencia de compra" title="Fácil desde que preguntás hasta que lo recibís" align="center"/><div className="mt-9"><Benefits/></div></section>
